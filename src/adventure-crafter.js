...
export default class AdventureCrafter {
    ...
    characterFromList() {
		...
		return {
			...
			"character": result,
			"characterIndexNum": characterIndex
		}    
    }
...
"Careless","Cautious","Sneaky","Intimidating","Powerful","Powerless","Hurt","Rough","Gentle","Caring","Principled","Arrogant","Curious","Supportive","Heroic"]
		...
		let descriptors = []
		if (num === null) {
		    let diceResult = utils.rollDice(1,100)
		    if (diceResult.total < 22) {
		        num = 2
		    } else {
		        num = 1
		    }
		...
		while (descriptors.length < num) {
			let new_descriptor = descriptorsTable[Math.floor(Math.random() * descriptorsTable.length)]
			if (!descriptors.includes(new_descriptor)) {
		...
		return {
			...
			"descriptorsString": utils.arrayToSentence(descriptors),
			"dice": false
		}
	}
	...
	characterTrait() {
		let traitTable = [
			[50, "The character is an individual"],
			[57, "The character is an organization"],
			[64, "The character is an object"],
			[71, "The character is connected to this plotline"],
			[78, "The character is not connected to this plotline"],
			[85, "The character assists in resolving this plotline"],
			[92, "The character hinders resolving this plotline"],
			[100, "The character is connected to an existing character"]]
			
		let GME_CHARACTER_TRAIT_DEFINITIONS = {
		...
		let diceResult = utils.rollDice(1,100)
        let trait = utils.getClosetRolledMatch(traitTable,diceResult.total)
	...
	characterIdentity(num=null) {
		let identitiesTable = ["Warrior","Healer","Protector","Assistant","Dependent","Ruler","Administrator","Victim","Scholar","Expert","Elite","Investigator","Criminal","Supporter","Helpless","Outsider","Mediator","Entertainer","Socialite","Athlete","Performer","Representative","Merchant","Trader","Creator","Artist","Servant","Laborer","Religious","Hunter","Leader","Fighter","Crafter","Thief","Radical","Executive","Thug","Guard","Guardian","Explorer","Hero","Villain","Deceiver","Engineer","Scout","Fixer","Wanderer","Subverter","Soldier","Law Enforcement","Scientist","Gatherer","Foreigner","Survivor","Gambler","Rogue","Farmer","Killer","Professional","Driver/Pilot",
"Student","Organizer","Deliverer","Lackey","Teacher","Exotic"]

		let identities = []
		if (num === null) {
		    let diceResult = utils.rollDice(1,100)
		    if (diceResult.total < 34) {
		        num = 2
		    } else {
		        num = 1
		    }
		...
		while (identities.length < num) {
			let new_identity = identitiesTable[Math.floor(Math.random() * identitiesTable.length)]
			if (!identities.includes(new_identity)) {
		...
		return {
			...
			"identitiesString": utils.arrayToSentence(identities),
			"dice": false
		}		
	}
	...
	generateCharacter() {
		return {
			'identity': this.characterIdentity(),
			'descriptor': this.characterDescriptor(),
